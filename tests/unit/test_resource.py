...
Command line: python3 -m pytest tests/unit/test_resource.py
"""
...
def test_repr_repr(resource):
    assert repr(resource) == f"{resource.name} ({resource.category} - {resource.manufacturer}) : total={resource.total}, allocated={resource.allocated}"
